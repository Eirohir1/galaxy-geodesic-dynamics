...
def generate_galaxy_no_cbh():
    masses = np.random.lognormal(np.log(0.5 * 1.989e30), 0.8, NUM_PARTICLES)
    masses[0] = 0.0  # REMOVE CENTRAL BLACK HOLE
    ...
    vel[0] = 0.0  # Remove any motion from massless center
    return pos, vel, masses

def run_mode_d_prime():
    pos, vel, mass = generate_galaxy_no_cbh()
...
def main():
    outdir = "geodesic_redteam_phase3_nocbh"
    os.makedirs(outdir, exist_ok=True)

    print("\n🚀 Running MODE D' (TAPER ONLY, NO CBH)...")
    pos, vel, Lz, retention, dispersion = run_mode_d_prime()
    plot_all(pos, vel, Lz, "mode_d_prime", outdir)

    with open(f"{outdir}/summary.txt", "w") as f:
        f.write(f"mode_d_prime: Retention = {retention:.2f}%, Dispersion = {dispersion:.2f} km/s\n")

    print("\n✅ Mode D' complete. Results saved in 'geodesic_redteam_phase3_nocbh/'.")
