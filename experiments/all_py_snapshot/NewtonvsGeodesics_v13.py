# NewtonvsGeodesics_v13.py
# GPU-accelerated realistic galaxy with three independent kernels:
#   - newton   : CBH + bulge + disk (no DM)
#   - geodesic : neighbor-overlap Venn-slice only (no CBH/bulge/disk) with auto-calibration
#   - gr       : GR-only (1PN) around the central BH (no bulge/disk/Newton)
#
# This version:
#   • Runs the simulation FIRST, then plots (so figures reflect the active kernel).
#   • Rotation-curve panel shows the active kernel’s effective curve from snapshot accelerations.
#   • Geodesic kernel auto-calibrates its coupling to match Newton’s radial accel near 8 kpc
#     (so its effect is visible and comparable).
#
# Outputs per run:
#   - gpu_galaxy_structure_{MODE}.png
#   - escapes_{MODE}.csv
...
G_KPC_KMS2_MSUN = 4.30091e-6  # (kpc * (km/s)^2) / M_sun
...
class RealisticGalaxy:
    ...
    Realistic Milky Way–like galaxy:
      ...
      - Hernquist bulge
      - Exponential disk (thin, optional bar)
      - Logarithmic spiral arms
      - Velocity dispersions & streaming
      - GPU-native arrays when available
    """
    def __init__(self,
                 num_stars=10000,
                 cbh_mass=4.15e6,     # M_sun
                 bulge_mass=2e10,     # M_sun
                 disk_mass=6e10,      # M_sun
    ...
    def _norm_r(self, x, y):
        return (cp.sqrt(x*x + y*y) if self.use_gpu else np.sqrt(x*x + y*y))

    def _rotation_curve_scalar(self, r):
        # numpy version (used during build)
        eps = 1e-3
        r = np.maximum(r, eps)
        ...
        return np.sqrt(v_cbh*v_cbh + v_bulge*v_bulge + v_disk*v_disk)

    def _rotation_curve_gpu(self, r_gpu):
        eps = 1e-3
        if self.use_gpu:
            r = cp.maximum(r_gpu, eps)
            v_cbh = cp.sqrt(self.G * self.cbh_mass / (r + 0.001))
            a = self.bulge_radius * 0.5
            m_enc_b = self.bulge_mass * r*r / (r + a)**2
            v_bulge = cp.sqrt(self.G * m_enc_b / r)
            m_enc_d = self.disk_mass * (1 - (1 + r/self.disk_scale_length) * cp.exp(-r/self.disk_scale_length))
            v_disk = cp.sqrt(self.G * m_enc_d / (r + 0.1))
            return cp.sqrt(v_cbh*v_cbh + v_bulge*v_bulge + v_disk*v_disk)
        else:
            r = np.maximum(r_gpu, eps)
            v_cbh = np.sqrt(self.G * self.cbh_mass / (r + 0.001))
            a = self.bulge_radius * 0.5
            m_enc_b = self.bulge_mass * r*r / (r + a)**2
            v_bulge = np.sqrt(self.G * m_enc_b / r)
    ...
    # --------- new helpers (snapshot accel & curves) ----------
    def _snapshot_accel(self, mode, influence_scale=0.6, coupling=1.0):
        """Acceleration at current positions for the chosen kernel (no stepping)."""
        if mode == "newton":
            return self._accel_newton_gpu()
        elif mode == "geodesic":
            return self._accel_geodesic_gpu(influence_scale=influence_scale, coupling=coupling)
        elif mode == "gr":
            return self._accel_gr_gpu()
        else:
            raise ValueError(mode)

    def _kernel_curve_from_snapshot(self, mode, influence_scale=0.6, coupling=1.0, nbins=24):
        """
        Build a rotation-curve-like profile from measured radial acceleration:
          v_eff(r) = sqrt( max(a_r, 0) * r )
        using current star positions and the active kernel.
        """
        import numpy as _np
        xp = cp if self.use_gpu else _np

        ax, ay, az = self._snapshot_accel(mode, influence_scale, coupling)
        r = self._norm_r(self.x, self.y)
        ar = -(ax * (self.x/(r+1e-9)) + ay * (self.y/(r+1e-9)))  # inward radial accel (positive)

        v_eff = xp.sqrt(xp.maximum(ar, 0.0) * r)

        r_cpu = cp.asnumpy(r) if self.use_gpu else r
        v_cpu = cp.asnumpy(v_eff) if self.use_gpu else v_eff
        rb = _np.linspace(0.2, self.galaxy_radius, nbins+1)
        rc, vc = [], []
        for i in range(nbins):
            m = (r_cpu >= rb[i]) & (r_cpu < rb[i+1])
            if m.any():
                rc.append(0.5*(rb[i]+rb[i+1]))
                vc.append(_np.median(v_cpu[m]))
        return _np.array(rc), _np.array(vc)

    def _auto_calibrate_geodesic(self, influence_scale=0.6, target_annulus=(7.0, 9.0)):
        """
        Choose a coupling so geodesic ar matches Newton ar around the solar circle.
        Returns coupling (float).
        """
        import numpy as _np
        # Newton radial accel at snapshot
        axN, ayN, _ = self._accel_newton_gpu()
        r = self._norm_r(self.x, self.y)
        arN = -(axN * (self.x/(r+1e-9)) + ayN * (self.y/(r+1e-9)))
        # Geodesic radial accel with coupling=1
        gx, gy, _ = self._accel_geodesic_gpu(influence_scale=influence_scale, coupling=1.0)
        arG1 = -(gx * (self.x/(r+1e-9)) + gy * (self.y/(r+1e-9)))

        if self.use_gpu:
            r = cp.asnumpy(r); arN = cp.asnumpy(arN); arG1 = cp.asnumpy(arG1)

        ring = (r >= target_annulus[0]) & (r <= target_annulus[1])
        aN = np.median(arN[ring]) if np.any(ring) else np.median(arN)
        aG = np.median(arG1[ring]) if np.any(ring) else np.median(arG1)
        aG = max(aG, 1e-12)  # avoid divide-by-zero

        coupling = 0.8 * (aN / aG)  # 80% of Newton to avoid overshoot
        print(f"[Geodesic] Auto-calibrated coupling ≈ {coupling:.3e} (annulus {target_annulus} kpc)")
        return coupling
    ...
    def plot_galaxy(self, mode_tag="newton", influence_scale=0.6, coupling=1.0):
        if self.use_gpu:
            x = cp.asnumpy(self.x); y = cp.asnumpy(self.y); z = cp.asnumpy(self.z)
            m = cp.asnumpy(self.masses)
            vx = cp.asnumpy(self.vx); vy = cp.asnumpy(self.vy); vz = cp.asnumpy(self.vz)
        ...
        # 3) Rotation curve — active kernel from snapshot + Newton reference
        ax3 = plt.subplot(2,3,3)
        r_samp = np.linspace(0.1, self.galaxy_radius, 128)
        v_ref = self._rotation_curve_scalar(r_samp)
        ax3.plot(r_samp, v_ref, linestyle='--', linewidth=1.5, alpha=0.6, label='Newton baseline (ref)')

        rc, vc = self._kernel_curve_from_snapshot(mode_tag, influence_scale, coupling)
        ax3.plot(rc, vc, linewidth=2, label=f'{mode_tag} kernel (post-sim)')
        ...
        for i in range(len(rbins)-1):
            ...
            if np.count_nonzero(mask) > 15:
                rc_b.append(0.5*(rbins[i]+rbins[i+1]))
                vb_b.append(np.median(v_circ[mask]))
        ax3.scatter(rc_b, vb_b, s=16, alpha=0.8, label='Stars (post-sim)')

        ax3.set_xlabel('Radius (kpc)'); ax3.set_ylabel('Velocity (km/s)')
        ax3.set_title('Rotation Curve (active kernel)')
    ...
    def _base_accel_gpu(self):
        """CBH + bulge + disk (Newtonian; no DM)."""
        if self.use_gpu:
    ...
    def _geodesic_overlap_accel_gpu(self, influence_scale=0.6, coupling=1.0, tile=4096):
        """
        Geodesic "raindrop" overlap (in-plane).
        Influence radius per star: R_i = influence_scale * (M_i)^(1/3)  [kpc]
        For neighbor j within R_i: a_i += coupling * G * |Mi - Mj| / d^2 * unit(i->j)
        """
        xp = cp if self.use_gpu else np
        N = self.num_stars
        x = self.x; y = self.y; m = self.masses

        Ri = influence_scale * xp.power(m, 1.0/3.0)
        ...
        for j0 in range(0, N, step):
            ...
            del xt, yt, mt, xi, yi, mi, Ri_i, dx, dy, d2, d, mask, mdiff, mag, ux, uy, ax, ay

        return ax_total, ay_total

    def _accel_newton_gpu(self):
        """Kernel 1: Newton-only CBH+bulge+disk."""
        return self._base_accel_gpu()

    def _accel_geodesic_gpu(self, influence_scale=0.6, coupling=1.0):
        """Kernel 2: Geodesic-only (neighbor overlap, in-plane)."""
        gx, gy = self._geodesic_overlap_accel_gpu(
    ...
    def _accel_gr_gpu(self):
        """
        Kernel 3: GR-only (1PN) for central BH; no bulge/disk/Newton terms.
        a ≈ a_N*[1 + (4GM/rc^2) - (v^2/c^2)] + (4GM/rc^2)*((v·r)/r^2)*v
        """
        xp = cp if self.use_gpu else np
        x, y, z = self.x, self.y, self.z
        vx, vy, vz = self.vx, self.vy, self.vz
        ...
        phi = GM / (r * c2)
        fac = (1.0 + 4.0*phi - v2/c2)

        vdotrhat = (vx*rx + vy*ry + vz*rz)
        corr = (4.0*phi) * (vdotrhat / r)

        ax = aN_mag * fac * rx + corr * vx
        ay = aN_mag * fac * ry + corr * vy
        az = aN_mag * fac * rz + corr * vz
        return ax, ay, az
    ...
    def simulate_orbits_gpu(self,
                            ...
                            report_tag="run"):
        """
        Three mutually-independent kernels:
          - 'newton'   : ONLY CBH+bulge+disk
          - 'geodesic' : ONLY neighbor-overlap kernel (in-plane); auto-calibrates coupling if None/<=0
          - 'gr'       : ONLY GR (1PN) around central BH
        Escape: r > 3 * galaxy_radius
        """
        xp = cp if self.use_gpu else np
        if not self.use_gpu:
        ...
        print("\n" + "="*60)
        print(f"GPU-ACCELERATED ORBITAL SIMULATION  [{mode.upper()}]")
        print("="*60)

        n_steps = int(round(time_myr / dt_myr))
        # 1 km/s = 1 kpc / 977.8 Myr
        dt_code = dt_myr / 977.8

        # Auto-calibrate geodesic coupling to Newton around 8 kpc
        if mode == "geodesic" and (coupling is None or coupling <= 0):
            coupling = self._auto_calibrate_geodesic(influence_scale=influence_scale)
        ...
        for step in range(n_steps):
            if mode == "newton":
                ax, ay, az = self._accel_newton_gpu()
            elif mode == "geodesic":
                ax, ay, az = self._accel_geodesic_gpu(influence_scale=influence_scale,
                                                      coupling=coupling)
            elif mode == "gr":
                ax, ay, az = self._accel_gr_gpu()
            else:
            ...
            if (step+1) % max(1, n_steps//10) == 0 or step == n_steps-1:
                ...
                esc_count = int((cp.sum(esc).get() if self.use_gpu else np.sum(esc)))
        ...
        if escaped_mask is None:
            escaped_mask = (cp.zeros(self.num_stars, dtype=cp.bool_) if self.use_gpu
                            else np.zeros(self.num_stars, dtype=bool))

        return (cp.asnumpy(escaped_mask) if self.use_gpu else escaped_mask,
                cp.asnumpy(self.initial_radius) if self.use_gpu else self.initial_radius,
                coupling if mode == "geodesic" else None)

# -----------------------------
# Utility: save escape CSV + print region stats
# -----------------------------
def save_escape_report(filename, escaped, r_init, x, y, vx, vy, masses):
    import csv
    N = len(escaped)
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Newton vs Geodesic vs GR (GPU) Galaxy Test")
    ap.add_argument('--mode', choices=['newton','geodesic','gr'], default='newton')
    ...
    ap.add_argument('--coupling', type=float, default=None)       # None/<=0 => auto-calibrate
    args = ap.parse_args()
    ...
    # Run integrator FIRST so plots reflect active kernel
    escaped, r_init, used_coupling = galaxy.simulate_orbits_gpu(
        mode=args.mode,
    ...
    # Pull CPU arrays for report/plot
    if galaxy.use_gpu:
    ...
    # Save escape report
    save_escape_report(f"escapes_{args.mode}.csv", escaped, r_init, x, y, vx, vy, masses)

    # Plot AFTER sim; rotation curve uses the active kernel + (for geodesic) the used coupling
    galaxy.plot_galaxy(mode_tag=args.mode,
                       influence_scale=args.influence_scale,
                       coupling=(used_coupling if used_coupling is not None else (args.coupling if args.coupling is not None else 1.0)))

    print("\n" + "="*60)
    print("RUN COMPLETE")
    print("="*60)
    if used_coupling is not None:
        print(f"(Geodesic coupling used: {used_coupling:.3e})")
    print("Next: run the other modes and compare escape summaries and curves.")
