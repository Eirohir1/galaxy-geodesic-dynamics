# NewtonvsGeodesics_v12.py
# GPU-accelerated realistic galaxy with three independent kernels:
#   - newton   : CBH + bulge + disk (no DM)
#   - geodesic : neighbor-overlap Venn-slice only (no CBH/bulge/disk)
#   - gr       : GR-only (1PN) around the central BH (no bulge/disk/Newton)
#
# Outputs per run:
#   - gpu_galaxy_structure_{MODE}.png  (diagnostic plot)
#   - escapes_{MODE}.csv               (per-star escape status)
#
# Defaults are set to run out-of-the-box on your GPU.
...
class RealisticGalaxy:
    ...
    def plot_galaxy(self, mode_tag="newton"):
        ...
        # 3) Rotation curve (baseline Newtonian curve only for reference)
    ...
    def simulate_orbits_gpu(self,
        ...
        Three mutually-independent kernels:
          ...
          - 'geodesic' : ONLY neighbor-overlap kernel (in-plane)
...
def main():
    ...
    print("RUN COMPLETE")
    print("="*60)
    print("Next: run the other modes and compare escape summaries and CSVs.")
