...
import time
import math  # Use standard math module for GPU

# === BETTER SCALED CONFIG ===
NUM_PARTICLES = 50000
NUM_STEPS = 200
DT = 1e11  # Time step
GALAXY_RADIUS = 3e19  # ~10 kpc
GALAXY_THICKNESS = 3e17  # ~1 kpc
BH_MASS = 4e36  # 2M☉ supermassive black hole
CUTOFF_RADIUS = 9.259e15  # 0.3 pc
        ...
        for j in range(N):
            if i != j:
                ...
                if dist_sq < cutoff * cutoff:
                    ...
                    strength = -G_const * BH_MASS / dist_sq
...
# === INITIALIZATION ===
print("🚀 Starting Geodesic Galaxy Simulation")
...
masses[0] = BH_MASS  # Supermassive black hole
...
for step in range(NUM_STEPS):
    ...
    # Update velocities and positions based on geodesic motion
    velocities += accelerations * DT
    positions += velocities * DT

runtime = time.time() - start_time
print(f"\n✅ Simulation complete in {runtime:.1f} seconds!")
...
ax1.set_title(f'Geodesic Galaxy - {np.sum(bound_mask):,} bound particles')
ax1.legend()
...
if len(bound_radii) > 20:
    ...
    ax2.set_title('Geodesic Rotation Curve')
